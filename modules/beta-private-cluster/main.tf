...
locals {
  ...
  cluster_alias_ranges_cidr = var.add_cluster_firewall_rules ? { for range in toset(data.terraform_remote_state.vpc.outputs.subnets["${var.region}/${var.subnetwork}"].secondary_ip_range[0]) : range.range_name => range.ip_cidr_range } : {}
